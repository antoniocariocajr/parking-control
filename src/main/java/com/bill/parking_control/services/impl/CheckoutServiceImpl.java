...
public class CheckoutServiceImpl implements CheckoutService {
    ...
    private final Clock clock; // facilita testes com tempo
    ...
    public PaymentResponseDTO checkout(String sessionId, PaymentMethod paymentMethod) {
        ...
        if (session.getStatus() != SessionStatus.ACTIVE) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Sessão já foi finalizada ou cancelada");
        }

        Tariff tariff = tariffRepository
                .findActiveByVehicleTypeAndDate(session.getVehicle().getType(), LocalDateTime.now(clock))
                .orElseThrow(
                        () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Tarifa não cadastrada para o tipo "
        ...
        // atualiza sessão
        session.setExitTime(LocalDateTime.now(clock));
